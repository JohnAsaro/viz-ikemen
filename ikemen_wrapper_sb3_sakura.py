...
CHAR_DEF = os.path.relpath(
    os.path.join(BASE_DIR, "sakura_env", "sakura_pots.def"),
    IKEMEN_DIR
)
CPU_DEF = os.path.relpath(
...
class IkemenEnv(gym.Env):
    ...
    def __init__(self, ai_level=1, screen_width=640, screen_height=480, show_capture=False, n_steps=-1, showcase=False, step_delay=0.0, headless = False, speed = 0, fps = 60, log_episode_result=True, instance_id="1A"):
        ...
        # Gym spaces
        self.action_space      = gym.spaces.Discrete(len(ACTIONS_SAKURA))
    ...
    def finish_episode(self):
        """
        -Mark row with that told Sakura what to do in the last episode as done.
        """
        conn = sqlite3.connect(self.DB_PATH)
        c = conn.cursor()
    ...
    def unpause(self):
        ...
        if ispaused == 1:
            self.reset() # Reset when unpausing
...
def test_ppo(env, model_path, n_episodes=10):
    ...
    for episode in range(n_episodes):  # For each episode
        ...
        while not done:  # While the game isn't done
            ...
            obs, reward, done, truncated = env.step(action)  # Take the action
            obs = obs.copy() # Work around negative stride error
            episode_reward += int(reward)  # Add the reward to the total reward
...
def make_env(instance_id = "test", n_steps=8192, ai_level=1, showcase=False, log_episode_result=False):
    def _init():
        return IkemenEnv(
            ai_level=ai_level,
            screen_width=80,
